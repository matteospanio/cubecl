...
impl<'a> Command<'a> {
    ...
    pub fn write_to_gpu(&mut self, descriptor: CopyDescriptor, data: Bytes) -> Result<(), IoError> {
        ...
        // Large contiguous uploads from unpinned host memory: the driver's async
        // copy from pageable memory stages internally and serially (well below
        // pinned DMA bandwidth), while page-locking a full-size staging buffer
        // costs O(size). Pipeline the transfer through two pooled pinned chunks
        // instead, overlapping each chunk's host memcpy with the previous
        // chunk's DMA.
        let rank = shape.len();
        let contiguous = rank <= 1 || strides[rank - 2] == shape[rank - 1];
        if contiguous
            && size > STAGE_CHUNK
            && !matches!(data.property(), AllocationProperty::Pinned)
        {
